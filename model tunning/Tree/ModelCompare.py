...
## Armar otro para poder comparar resultados de Cross-Validation y Train.
class modelCompareClass:

    # Al crearse una instancia de la clase se genera el siguiente dataset vacío.
    def __init__(self, predictions = "Test"):
        columnas = ['Model', 'Train Accuracy', f'{predictions} Accuracy', 'Train Sen', f'{predictions} Sen', 'Train Spec', f'{predictions} Spec', 'Train AUC', f'{predictions} AUC']
    ...
    # Este método permite graficar de manera rápida la comparación de resultados entre alguna métrica en Train y self.predictions (def: Test o Validation)
    # El argumento metric indica con qué métrica se evalúa: Accuracy (def), Sen, Spec, AUC.
    # Compara todos los modelos guardados en el dataframe self.results y los ordena según métrica self.predictions (Test).
    def plotMetric(self, metric = "Accuracy", ylim = [0,1]):
        df_plot = self.results.sort_values(f'{self.predictions} {metric}', ascending = False)

        plt.plot(df_plot['Model'], df_plot[f'Train {metric}'])
        plt.plot(df_plot['Model'], df_plot[f'{self.predictions} {metric}'], color = 'red')
        plt.scatter(df_plot['Model'], df_plot[f'Train {metric}'])
        plt.scatter(df_plot['Model'], df_plot[f'{self.predictions} {metric}'], color = 'red')
        plt.title(f'{metric} Train and {self.predictions}')
        plt.xlabel('Model')
        plt.ylabel('{metric}')
        plt.xticks(rotation = 45)
        plt.legend(['Train', f'{self.predictions}'], loc = 'lower right')
        plt.ylim(ylim)
        plt.show()
    ...
    def modelReportClass(self, name, model, X_train, y_train, X_test, y_test, heatmap = True, ROC = True, classReport = False, addModel = False, probas = True):
        ...
        print('-----------------------------------')
        print('\n')
        print(f'-----------{self.predictions} Metrics-----------')
